...
// degrees-to-radians:
__device__ float Radians(float d) { return (M_PI / 180.f) * d; }

__global__ void MonteCarlo(IN float *dtxs, IN float *dtys, IN float *dtxvs,
                           IN float *dsvs, IN float *dsths, IN float *dhalflens,
                           OUT int *dhits) {
