kernel void MultReduce(global const float *A, global const float *B,
                       local float *prods, global float *sums)
{
    ...
    prods[tnum] = A[gid] * B[gid]; // multiply the two arrays together
    ...
    for (int offset = 1; offset < numItems; offset *= 2)
    {
        ...
        if ((tnum & mask) == 0)       // bit anding here tells us which threads need to do work now
        {
