...
// these are the same Platform and Device global variables from the sample
// OpenCL code: you don't need them declared both times
...
// coment this #define out if you want to use this file as subroutines instead
// of a main program:
#define MAIN_PROGRAM_TO_TEST

#ifdef MAIN_PROGRAM_TO_TEST
// compile with:
// g++ -o printinfo printinfo.cpp
// /usr/local/apps/cuda/10.1/lib64/libOpenCL.so.1.1 -lm -fopenmp
...
void SelectOpenclDevice() {
  ...
  //	3. any gpu
  //	4. an intel cpu
  //	(this will need to be adjusted when the intel discrete card, arc, hits
  //the market)
  ...
  for (int p = 0; p < (int)numPlatforms; p++) {
    ...
    for (int d = 0; d < (int)numDevices; d++) {
      ...
      {
        if (bestDeviceType == CL_DEVICE_TYPE_CPU) // holding a cpu already --
                                                  // switch to a gpu if possible
        {
