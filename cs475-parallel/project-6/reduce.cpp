...
int main(int argc, char *argv[]) {
  ...
  float *host_sums = new float[NUM_WORK_GROUPS];

  // fill the host memory buffers:

  for (int i = 0; i < NUM_ELEMENTS; i++) {
    host_A[i] = host_B[i] = (float)sqrt((double)i);
  }

  size_t dataSize = NUM_ELEMENTS * sizeof(float);
  size_t sumsSize = NUM_WORK_GROUPS * sizeof(float);
  ...
  cl_mem device_sums =
      clCreateBuffer(context, CL_MEM_WRITE_ONLY, sumsSize, NULL, &status);
  if (status != CL_SUCCESS)
  ...
  // local array for intermediate sums
  status = clSetKernelArg(kernel, 2, LOCAL_SIZE * sizeof(float), NULL);
  if (status != CL_SUCCESS)
    fprintf(stderr, "clSetKernelArg failed (3)\n");

  // global smaller array for workgroup products
  status = clSetKernelArg(kernel, 3, sizeof(cl_mem), &device_sums);
  if (status != CL_SUCCESS)
  ...
  status = clEnqueueReadBuffer(cmdQueue, device_sums, CL_TRUE, 0, sumsSize,
                               host_sums, 0, NULL, NULL);
  if (status != CL_SUCCESS)
...
#ifndef CSV
  fprintf(stderr, "%8d\t%4d\t%10d\t%10.3lf GigaMultsPerSecond\t%.0f sum\n",
          NUM_ELEMENTS, LOCAL_SIZE, NUM_WORK_GROUPS,
          (double)NUM_ELEMENTS / (time1 - time0) / 1000000000., sum);
#else
  printf("%d,%d,%d,%lf,%.0f\n", NUM_ELEMENTS, LOCAL_SIZE, NUM_WORK_GROUPS,
         (double)NUM_ELEMENTS / (time1 - time0) / 1000000000., sum);
#endif
  ...
  clReleaseMemObject(device_sums);
